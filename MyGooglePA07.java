...
public class MyGooglePA07 {
   ...
   **/
   public static String[] findLinks(String[] lines){
       String[] linksArray = new String[10000];
       int positionInLinkArray = 0;
       for(int i = 0; i< lines.length; i++){
           if(lines[i].contains("<a href=")){
               linksArray[positionInLinkArray] = getLink(lines[i]);
               positionInLinkArray++;
           }
       }
       return Arrays.copyOfRange(linksArray,0,positionInLinkArray);
   }
   ...
   **/
   public static String getLink(String line){
       int s = line.indexOf("<a href=");
       int startOfLink = s+9;
       int endOfLink = line.indexOf("\"", startOfLink);
       if(endOfLink > startOfLink){
           return line.substring(startOfLink,endOfLink);
       }
       else{
           return line.substring(startOfLink);
       }
   }
   ...
   **/
   public static void printMatches(String searchPhrase, String[] matches){
       for(int i=0; i<matches.length; i++){
           int indexOfPhrase = matches[i].indexOf(searchPhrase);
           int lenOfPhrase = searchPhrase.length();
           int lenOfLineAfterPhrase = matches[i].length() - lenOfPhrase - indexOfPhrase;

           if(indexOfPhrase > 50 && lenOfLineAfterPhrase > 50){
               System.out.println(matches[i].substring(indexOfPhrase-50, indexOfPhrase+lenOfPhrase+50));
           }
           else if(indexOfPhrase > 50 && lenOfLineAfterPhrase < 50){
               System.out.println(matches[i].substring(indexOfPhrase-50));
           }
           else if(indexOfPhrase < 50 && lenOfLineAfterPhrase > 50){
               System.out.println(matches[i].substring(0, indexOfPhrase+lenOfPhrase+50));
           }
           else{
   ...
   **/
   public static String getRandomLink(String[] links){
       Random r = new Random();
       int number = r.nextInt(links.length);
       return links[number];
   }
}
